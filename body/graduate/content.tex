...
然而，在当前的三维内容生产管线中，\textbf{“虚拟运镜控制”}（Virtual Camera Control）依然是一个极具挑战性的技术瓶颈 \cite{christie2008camera}。这本质上是一个典型的\textbf{“语义鸿沟”}（Semantic Gap）问题：艺术家和导演习惯使用抽象的感性语言（例如“具有压迫感的仰视”、“希区柯克式变焦”）来描述镜头意图 \cite{arijon1976grammar, mascelli1965five}；而渲染引擎的摄像机系统则依赖于底层的、精确的工程参数（如世界坐标位置 $T$、四元数旋转 $q$ \cite{shoemake1985animating}、视场角 $FOV$）。这种不匹配导致创作者往往需要陷入漫长的“试错循环” \cite{lino2015intuitive}。

此外，现有的 AIGC 运镜研究多局限于\textbf{“静态单帧”}的构图生成，缺乏对\textbf{“动态时序”}运镜的有效支持。虽然 Sora \cite{liu2024sora}、Gen-2 \cite{esser2023structure} 等视频生成模型能够生成运镜画面，但其相机轨迹是隐式的、不可控且不可编辑的 \cite{he2024cameractrl}。因此，如何利用 AIGC 技术实现从“单帧构图”到“动态轨迹”的全流程智能化控制，是本研究致力于解决的核心问题。
...
\begin{enumerate}
    ...
    针对传统运镜控制的痛点，本文提出了一种“图生参”的跨模态逆向求解方案。
    首先，受 LLaVA \cite{liu2023visual} 和 BLIP \cite{li2022blip} 等多模态模型启发，本文构建了基于 LLM 的提示词优化模块，并引入 ControlNet \cite{zhang2023adding} 施加空间几何约束。这解决了传统方法中自然语言指令难以精确映射的问题 \cite{radford2021learning}。
    其次，针对传统轴对齐包围盒（AABB）的局限性，本文创新性地提出基于**最小外接矩形（OBB）** \cite{xie2021oriented, yang2021r3det} 的逆向投影算法。基于多视图几何理论 \cite{hartley2003multiple, szeliski2022computer}，该算法不仅能利用相似三角形反解拍摄距离，更首次利用 OBB 旋转角实现了摄像机翻滚角（Roll）的自动解算。这一方法通过引入特征提取技术 \cite{suzuki1985topological, lowe2004distinctive}，有效填补了现有反求算法在艺术化倾斜构图方面的空白。
...
本节详细推导从二维 OBB 特征 $\{ c_x, c_y, h_{box}, \theta_{box} \}$ 到三维摄像机 6-DoF 位姿参数的数学映射过程。解算目标是确定摄像机在世界坐标系下的位置 $P_{cam}$ 和旋转矩阵 $R_{cam}$。此过程基于理想的针孔成像模型 \cite{hartley2003multiple}，并结合了多视图几何中的透视投影原理。
...
\section{基于前向向量启发的三阶贝塞尔路径规划}

传统的线性插值（Linear Interpolation, Lerp）生成的轨迹是直线，这在视觉上显得机械且生硬。为了模拟推车（Dolly）或摇臂（Crane）的弧形运动轨迹，本研究采用三阶贝塞尔曲线进行空间位置规划。
...
\subsection{启发式控制点生成策略}
如何自动确定 $P_1$ 和 $P_2$ 的位置是算法的关键。若需用户手动调整，则违背了“全自动化”的设计初衷。
本研究提出一种基于**摄像机前向向量（Forward Vector）**的启发式生成策略。根据电影摄影经验 \cite{arijon1976grammar}，运镜轨迹通常会顺着起始镜头的朝向延伸一段距离，再缓慢过渡到终止位置。

设起始帧的前向向量为 $\vec{F}_A$，终止帧的前向向量为 $\vec{F}_B$，两点间的直线距离为 $L = \|P_B - P_A\|$。我们将控制点设定为：
\begin{align}
    P_1 &= P_A + \alpha \cdot L \cdot \vec{F}_A \\
    P_2 &= P_B - \beta \cdot L \cdot \vec{F}_B
\end{align}
其中 $\alpha, \beta \in [0.3, 0.5]$ 为曲率系数。
该策略的几何意义在于：强迫轨迹在出发时沿 $K_{start}$ 的视线方向运动，在到达时沿 $K_{end}$ 的视线反方向切入。如图 \ref{fig:bezier_schematic} 所示，这种设计能够生成极其自然的“环绕”或“跟随”曲线，有效避免了穿模和视线丢失问题。

\begin{figure}[H]
    \centering
    % 占位符：系统结构框图.png (未来替换为贝塞尔曲线控制点示意图)
    
    \includegraphics[width=0.8\textwidth]{figure/系统结构框图.png}
    \caption{基于前向向量启发的三阶贝塞尔曲线控制点生成示意图。$P_1, P_2$ 由视线方向 $\vec{F}$ 自动计算得出。}
    \label{fig:bezier_schematic}
\end{figure}
...
\subsection{定性与定量结果分析}
实验结果如图 \ref{fig:static_experiment} 所示。在简单的车辆场景中，算法能够精确还原距离与视角。特别值得注意的是场景 (C)，得益于 OBB 算法对 $\theta_{box}$ 的提取，系统成功识别出 $15^\circ$ 的画面倾斜，并驱动摄像机产生了相应的 Roll 角，而传统的 LookAt 算法在此场景下完全失效（Roll 始终为 0）。
...
定量数据如表 \ref{tab:iou_results} 所示。本系统在所有测试集上的平均 IoU 达到 0.82，显著优于未引入 OBB 特征的基准方法（Baseline）。
...
为了验证第四章提出的“贝塞尔路径规划与 Slerp 插值算法”在动态运镜中的表现，本节对比了线性插值（Linear）与本文算法（Ours）生成的轨迹质量。

\subsection{角速度连续性测试}
运镜的“电影感”很大程度上取决于运动的流畅性。我们设定一个 $180^\circ$ 的环绕拍摄任务（Orbit Shot），分别记录相机在 $t \in [0,1]$ 过程中的角速度变化 $\omega(t)$。

如图 \ref{fig:angular_velocity} 所示：
\begin{itemize}
    \item **线性插值组**：角速度曲线呈现锯齿状，且在关键帧处存在剧烈的速度突变（导数不连续），视觉上表现为相机的“机械顿挫”。
    \item **本文算法组**：得益于 Slerp 插值与缓动函数（Easing Function）的共同作用，角速度曲线呈平滑的钟形（Bell Curve）。相机平稳起步，在中间达到最大速度，最后柔和停止。这种运动特性高度拟合了专业摄影云台的物理惯性 \cite{wang2024motionctrl}。
\end{itemize}
...
为了验证 ControlNet 几何约束模块的必要性，我们进行了一组消融实验。在关闭 ControlNet 的情况下，仅依靠 Prompt 生成参考图。
实验发现，虽然生成的图像风格符合描述，但物体的主体朝向往往随机化（例如 Prompt 要求“侧视”，生成的却是“正视”）。这导致 OBB 提取的特征与 3D 场景完全不匹配，最终的 IoU 精度下降至 0.35 以下。这充分证明了在“跨模态逆向解算”任务中，引入几何条件约束是保证系统鲁棒性的前提 \cite{zhang2023adding}。
...
\begin{enumerate}
    ...
    \item \textbf{静态精度方面}：OBB 逆向算法在处理复杂构图（特别是荷兰角）时表现优异，平均 IoU 达到 0.82。
    \item \textbf{动态质量方面}：基于贝塞尔与 Slerp 的轨迹生成算法成功消除了运动抖动，赋予了运镜以专业的电影质感。
\end{enumerate}
